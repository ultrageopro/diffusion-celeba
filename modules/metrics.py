...
class MetricsCounter:
    ...
    def _init_transforms(self) -> None:
        ...
        def denormalize_fn(t: torch.Tensor) -> torch.Tensor:
            # t ожидается в диапазоне [-1, 1]
            t = (t + 1) * 0.5  # перевод в диапазон [0, 1]
            if t.dim() == 4:
                # Для батча: (N, C, H, W) -> (N, H, W, C)
                t = t.permute(0, 2, 3, 1)
            elif t.dim() == 3:
                # Для одиночного изображения: (C, H, W) -> (H, W, C)
                t = t.permute(1, 2, 0)
            t = t * 255
            t = t.clamp(0, 255).to(torch.uint8)
            return t  # возвращаем тензор, удобный для дальнейших преобразований
    ...
    def count_metrics(
        ...
        for test_batch, target_batch in progress:
            ...
            target_denorm = self.denormalize(target_batch).float() / 255.0
            ...
            psnr_val = peak_signal_noise_ratio(generated, target_denorm, data_range=1.0).item()
            ssim_val = structural_similarity_index_measure(generated, target_denorm, data_range=1.0).item()
        ...
        print("Метрики сохранены в", save_path)
